...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Player", "loadDepotLocker", LuaScriptInterface::luaPlayerLoadDepotLocker);
	registerMethod("Player", "unloadDepotLocker", LuaScriptInterface::luaPlayerUnloadDepotLocker);
	...
	registerMethod("Player", "getStringStorageValue", LuaScriptInterface::luaPlayerGetStringStorageValue);
	registerMethod("Player", "addStringStorageValue", LuaScriptInterface::luaPlayerSetStringStorageValue);
	...
	registerMethod("Player", "getMaxDepotItems", LuaScriptInterface::luaPlayerGetMaxDepotItems);
	...
	registerMethod("Monster", "addWaitToDo", LuaScriptInterface::luaMonsterAddWaitToDo);
	registerMethod("Monster", "addGoToDo", LuaScriptInterface::luaMonsterAddGoToDo);
	registerMethod("Monster", "addAttackToDo", LuaScriptInterface::luaMonsterAddAttackToDo);
	registerMethod("Monster", "castAttackSpells", LuaScriptInterface::luaMonsterCastAttackSpells);
	registerMethod("Monster", "castDefensiveSpells", LuaScriptInterface::luaMonsterCastDefensiveSpells);
	registerMethod("Monster", "spawnSummons", LuaScriptInterface::luaMonsterSpawnSummons);
	registerMethod("Monster", "startToDo", LuaScriptInterface::luaMonsterStartToDo);
...
int LuaScriptInterface::luaDoAddOfflinePlayerMoney(lua_State* L)
{
	...
	if (!IOLoginData::loadPlayerByGUID(&player, guid)) {
		pushBoolean(L, false);
		return 1;
	}
	...
	g_game.addMoney(depotLocker, money, FLAG_NOLIMIT);
...
int LuaScriptInterface::luaGameCreateTile(lua_State* L)
{
	...
	Tile* tile = g_game.map.getTile(position);
	if (!tile) {
		tile = new Tile(position.x, position.y, position.z);
...
int LuaScriptInterface::luaGameStartRaid(lua_State* L)
{
	...
	const RaidPtr& raid = g_game.raids.getRaidByName(raidName);
...
{
	// player:getDepotLocker(depotId, force)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	uint32_t depotId = getNumber<uint32_t>(L, 2);
	bool force = getBoolean(L, 3, true);
	DepotLocker* depotLocker = player->getDepotLocker(depotId, force);
	if (depotLocker) {
...
int LuaScriptInterface::luaPlayerLoadDepotLocker(lua_State* L)
{
	// player:loadDepotLocker(depotId)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint32_t depotId = getNumber<uint32_t>(L, 2);
		player->loadDepotLocker(depotId);
...
int LuaScriptInterface::luaPlayerUnloadDepotLocker(lua_State* L)
{
	// player:unloadDepotLocker(depotId)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint32_t depotId = getNumber<uint32_t>(L, 2);
		player->unloadDepotLocker(depotId);
...
int LuaScriptInterface::luaPlayerGetStringStorageValue(lua_State* L)
{
	// player:getStringStorageValue(key)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	std::string key = asLowerCaseString(getString(L, 2));
	std::string value;
	if (player->getStringStorageValue(key, value)) {
		pushString(L, value);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerSetStringStorageValue(lua_State* L)
{
	// player:setStringStorageValue(key, value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		std::string key = asLowerCaseString(getString(L, 2));
		std::string value = asLowerCaseString(getString(L, 3));
		player->addStringStorageValue(key, value);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerChannelSay(lua_State* L)
{
	...
	uint16_t channelId = getNumber<uint16_t>(L, 5);
	player->sendToChannel(0, speaker, type, text, channelId);
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerGetMaxDepotItems(lua_State* L)
{
	// player:getMaxDepotItems()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getMaxDepotItems());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaMonsterAddWaitToDo(lua_State* L)
{
	// monster:addWaitToDo(time)
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		uint32_t time = getNumber<uint32_t>(L, 2);
		if (time == 0) {
			reportErrorFunc(L, "Invalid time");
			pushBoolean(L, false);
			return 1;
		}
...
int LuaScriptInterface::luaMonsterAddGoToDo(lua_State* L)
{
	// monster:addGoToDo(direction)
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int LuaScriptInterface::luaMonsterAddAttackToDo(lua_State* L)
{
...
int LuaScriptInterface::luaMonsterCastAttackSpells(lua_State* L)
{
	// monster:castAttackSpells()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int LuaScriptInterface::luaMonsterCastDefensiveSpells(lua_State* L)
{
	// monster:castDefensiveSpells()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int LuaScriptInterface::luaMonsterSpawnSummons(lua_State* L)
{
...
int LuaScriptInterface::luaMonsterStartToDo(lua_State* L)
{
	// monster:startToDo()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int LuaScriptInterface::luaHouseSave(lua_State* L)
{
	...
	pushBoolean(L, IOMap::saveHouseTVPFormat(house));
	return 1;
}
...
{
	// get: monsterType:isHostile()
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		pushBoolean(L, monsterType->info.baseSkill != 0);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaPartyGetInvitees(lua_State* L)
{
	...
	Party* party = getUserdata<Party>(L, 1);
	if (party) {
		size_t totalInvites = 0;
		std::vector<Player*> invitees;

		for (const uint32_t& guid : party->getInvitees()) {
			if (Player* player = g_game.getPlayerByGUID(guid)) {
				invitees.push_back(player);
				++totalInvites;
			}
		}

		lua_createtable(L, totalInvites, 0);

		int index = 0;
		for (Player* player : invitees) {
			pushUserdata<Player>(L, player);
			setMetatable(L, -1, "Player");
			lua_rawseti(L, -2, ++index);
		}
