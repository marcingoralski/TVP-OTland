...
struct RuleViolation
{
	...
	RuleViolation(uint32_t _reporterId, const std::string& _text) :
	    reporterId(_reporterId), gamemasterId(0), text(_text), pending(true)
	{}
...
static constexpr int32_t EVENT_LIGHTINTERVAL = 1000;
...
static constexpr int32_t EVENT_COMMUNICATION_INTERVAL = 5000;
...
public:
	...
	/// Attempt to move a creature in the given direction.
	///	This function is called by the action queue system of the creature.
	/// \param creature
	/// \param direction
	/// \param flags
	...
	/// <summary>
	/// Remove player money from their inventory.
	/// This function follows RealOTS way of removing money from a player's inventory.
	/// </summary>
	/// <param name="player"></param>
	/// <param name="money"></param>
	/// <returns></returns>
	bool removePlayerMoney(Player* player, uint64_t money);
	...
	void creatureTurn(Creature* creature, Direction dir) { internalCreatureTurn(creature, dir); }
	...
	bool searchFreeField(Creature* creature, uint16_t& x, uint16_t& y, uint8_t& z, int32_t distance, bool jump,
	                     bool allowHouses);
	bool searchLoginField(Creature* creature, uint16_t& x, uint16_t& y, uint8_t& z, int32_t distance, bool player,
	                      bool allowHouses = false);
	...
	void storePlayerName(uint32_t id, const std::string& str) { preloadedPlayerGuids[id] = str; }
	std::string getStoredPlayerNameByGUID(uint32_t guid) const
	{
		auto it = preloadedPlayerGuids.find(guid);
		if (it == preloadedPlayerGuids.end()) {
			return {};
		}
		return it->second;
	}
	...
	BlockType_t combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense,
	                           bool checkArmor, bool field, bool ignoreResistances = false, bool meleeHit = false);
	...
	static void addAnimatedText(const SpectatorVec& spectators, const Position& pos, TextColor_t textColor,
	...
	std::vector<Tile*> getTilesToRefresh() const { return tilesToRefresh; }
	void addTileToRefresh(Tile* tile) { tilesToRefresh.push_back(tile); }
	...
	std::vector<Tile*> getTilesToSave() const { return tilesToSave; }
	void addTileToSave(Tile* tile) { tilesToSave.push_back(tile); }

	bool isMapSavingEnabled() const { return allowMapSave; }
	void changeAllowMapSave(bool value) { allowMapSave = value; }
	...
	void toggleSendPlayersToTemple(bool v) { sendPlayersToTemple = v; }

	uint32_t recordStatement(uint32_t guid, uint32_t mode, uint32_t channel, const std::string& text);
	void recordListener(uint32_t statementId, uint32_t guid);
...
private:
	...
	std::unordered_map<uint32_t, std::string> preloadedPlayerGuids;
	...
	uint8_t oldLightLevel = LIGHT_DAY;
	uint8_t oldLightColor = 215;
	int16_t worldTime = 0;
