...
{
public:
	void parseList(const std::string& value);
...
using HouseTileList = std::list<Tile*>;
...
public:
	...
	void setSize(uint32_t size) { itemTileSize = size; }
	uint32_t getSize() const { return itemTileSize; }
	...
	void clearTiles() { houseTiles.clear(); }
	const HouseTileList& getTiles() const { return houseTiles; }

	void clearDoors() { doorSet.clear(); }
