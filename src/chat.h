...
using InvitedSet = std::unordered_set<uint32_t>;
...
public:
	...
	virtual const InvitedSet* getInvitedUsers() const { return nullptr; }
...
public:
	...
	void invitePlayer(const Player& player, uint32_t guid);
	void excludePlayer(const Player& player, uint32_t guid);

	bool removeInvite(uint32_t guid);

	void closeChannel() const;

	const InvitedSet* getInvitedUsers() const override { return &invites; }
