...
void Party::disband()
{
	...
	for (const uint32_t& guid : inviteList) {
...
bool Party::passPartyLeadership(Player* player)
{
	...
	for (const uint32_t& guid : inviteList) {
		if (Player* invitee = g_game.getPlayerByGUID(guid)) {
...
bool Party::joinParty(Player& player)
{
	...
	auto it = std::find(inviteList.begin(), inviteList.end(), player.getGUID());
...
bool Party::invitePlayer(Player& player)
{
	...
	inviteList.push_back(player.getGUID());
...
{
	return std::find(inviteList.begin(), inviteList.end(), player->getGUID()) != inviteList.end();
}
...
void Party::broadcastPartyMessage(MessageClasses msgClass, const std::string& msg, bool sendToInvitations /*= false*/)
{
	...
	if (sendToInvitations) {
		for (const uint32_t& guid : inviteList) {
			if (Player* player = g_game.getPlayerByGUID(guid)) {
				player->sendTextMessage(msgClass, msg);
			}
		}
	}
}
