...
void PrivateChatChannel::invitePlayer(const Player& player, uint32_t guid) { invites.insert(guid); }

void PrivateChatChannel::excludePlayer(const Player& player, uint32_t guid)
{
	if (!removeInvite(guid)) {
		return;
	}

	if (Player* excludePlayer = g_game.getPlayerByGUID(guid)) {
...
bool ChatChannel::talk(const Player& fromPlayer, SpeakClasses type, const std::string& text)
{
	...
	uint32_t statementId = g_game.recordStatement(fromPlayer.getGUID(), type, getId(), text);

	for (const auto& it : users) {
		it.second->sendToChannel(statementId, &fromPlayer, type, text, id);
		g_game.recordListener(statementId, it.second->getGUID());
	}
	return true;
}
...
bool Chat::talkToChannel(const Player& player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if ((type == TALKTYPE_CHANNEL_R2 || type == TALKTYPE_CHANNEL_R1) &&
	    !player.hasFlag(PlayerFlags::PlayerFlag_CanTalkRedChannel)) {
		type = TALKTYPE_CHANNEL_Y;
	}
