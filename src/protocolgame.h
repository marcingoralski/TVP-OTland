...
static constexpr int32_t CLIENT_TERMINAL_WIDTH = 18;
static constexpr int32_t CLIENT_TERMINAL_HEIGHT = 14;
...
private:
	...
	bool isVisible(int32_t x, int32_t y, int32_t z) const;
	...
	void addGameTask(Callable&& function, Args&&... args)
	{
		g_dispatcher.addTask(
		    createTask(std::bind(std::forward<Callable>(function), &g_game, std::forward<Args>(args)...)));
	}
	...
	{
		g_dispatcher.addTask(
		    createTask(delay, std::bind(std::forward<Callable>(function), &g_game, std::forward<Args>(args)...)));
	}
