...
void House::setOwner(uint32_t guid, bool updateDatabase /* = true*/, Player* player /* = nullptr*/)
{
	...
	// send items to depot
	if (guid == 0 || g_config.getBoolean(ConfigManager::HOUSE_TRANSFEROWNERSHIP_TRANSFERITEMS)) {
		if (player) {
	...
	if (g_config.getBoolean(ConfigManager::HOUSE_TRANSFEROWNERSHIP_CLEANBEDS)) {
	...
	if (g_config.getBoolean(ConfigManager::HOUSE_TRANSFEROWNERSHIP_CLEARDOORS)) {
	...
	if (guid != 0) {
		...
		if (const std::string name = IOLoginData::getNameByGuid(guid); !name.empty()) {
			...
			if (updateDatabase) {
				...
				strRentPeriod = asLowerCaseString(g_config.getString(ConfigManager::HOUSE_RENT_PERIOD));
				if (strRentPeriod == "yearly") {
				...
				if (Player* playerOnline = g_game.getPlayerByGUID(guid)) {
					...
					depotLocker->addItemFront(letter);
				} else {
					Player newOwner(nullptr);
					if (IOLoginData::loadPlayerByGUID(&newOwner, guid)) {
						DepotLocker* depotLocker = newOwner.getDepotLocker(townId, true);
						depotLocker->addItemFront(letter);

						IOLoginData::savePlayer(&newOwner);
					}
				}
			}
		}
	...
	if (g_config.getBoolean(ConfigManager::HOUSE_DOOR_SHOW_PRICE)) {
		for (const auto& it : doorSet) {
			it->setSpecialDescription(fmt::format(
			    ...
			    owner == 0 ? fmt::format(" It costs {:d} gold coins.", (housePrice * getTiles().size())) : ""));
		}
	} else {
		for (const auto& it : doorSet) {
			it->setSpecialDescription(fmt::format("It belongs to house '{:s}'. {:s} owns this house.", houseName,
...
bool House::kickPlayer(Player* player, Player* target)
{
	...
	if (target->getTile()->getHouse() != this) {
		return false;
	}
...
void House::setAccessList(uint32_t listId, const std::string& textlist)
{
	...
	for (Tile* tile : houseTiles) {
		if (const CreatureVector* creatures = tile->getCreatures()) {
			for (int32_t i = creatures->size(); --i >= 0;) {
				if (Player* player = (*creatures)[i]->getPlayer(); player && !isInvited(player)) {
...
{
	if (townId == 0 || owner == 0 || transferred) {
		return false;
	}

	if (Player* player = g_game.getPlayerByGUID(owner)) {
		transferToDepot(player);
	} else {
		Player tmpPlayer(nullptr);
		if (!IOLoginData::loadPlayerByGUID(&tmpPlayer, owner)) {
			return false;
		}
...
bool House::transferToDepot(Player* player) const
{
	...
	DepotLocker* depotLocker = player->getDepotLocker(townId, true);
	for (Item* item : moveItemList) {
		g_game.internalMoveItem(item->getParent(), depotLocker, INDEX_WHEREEVER, item, item->getItemCount(), nullptr,
		                        FLAG_NOLIMIT);
	}
...
void House::addBed(BedItem* bed)
{
	...
	if (bed->getSleeper()) {
		g_game.setBedSleeper(bed, bed->getSleeper());
	}
}
...
void AccessList::parseList(const std::string& value)
{
	playerList.clear();
	guildRankList.clear();
	allowEveryone = false;
...
void Houses::payHouses(RentPeriod_t rentPeriod) const
{
	...
	for (const auto& it : houseMap) {
		...
		if (!IOLoginData::loadPlayerByGUID(&player, ownerId)) {
		...
		if (eligibleToPayRent) {
			...
			} else if (DepotLocker* const depotLocker = player.getDepotLocker(townId, true)) {
				if (!depotLocker->getParent()) {
					depotLocker->setParent(&player);
				}

				if (g_game.removeMoney(depotLocker, houseRent, FLAG_NOLIMIT)) {
					paidRent = true;
				}
			}
		}
		...
		if (paidRent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				letter->setText(fmt::format(
				    "Warning! \nThe {:s} rent of {:d} gold for your house \"{:s}\" is payable. Have it available within {:d} days, or you will lose this house.",
				    period, houseRent, house->getName(), daysLeft));
				player.getDepotLocker(townId, true)->addItem(letter);
