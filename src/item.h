...
private:
	...
	const static uint32_t intAttributeTypes =
	    ...
	    ITEM_ATTRIBUTE_DOORID | ITEM_ATTRIBUTE_DECAYTO | ITEM_ATTRIBUTE_ATTACK_SPEED | ITEM_ATTRIBUTE_KEYNUMBER |
	    ITEM_ATTRIBUTE_KEYHOLENUMBER | ITEM_ATTRIBUTE_DOORLEVEL | ITEM_ATTRIBUTE_DOORQUESTNUMBER |
...
public:
	...
	void setDoorLevel(uint16_t n) { setIntAttr(ITEM_ATTRIBUTE_DOORLEVEL, n); }
	...
	void setKeyNumber(uint16_t n) { setIntAttr(ITEM_ATTRIBUTE_KEYNUMBER, n); }
	...
	virtual void serializeTVPFormat(ScriptWriter& script) const;

	// Binary Serialization
	virtual Attr_ReadValue readAttr(AttrTypes_t attr, PropStream& propStream);
	bool unserializeAttr(PropStream& propStream);
	void serializeTVPFormat(PropWriteStream& propWriteStream) const;
	...
	{
		return (isPickupable() ? std::numeric_limits<uint8_t>::max()
		                       : 2); // there is actually no limit in 7.7 for takable objects
	}
	...
	DepotLocker* getHoldingDepot();
	const DepotLocker* getHoldingDepot() const;
