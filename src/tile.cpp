...
ReturnValue Tile::queryAdd(int32_t, const Thing& thing, uint32_t, uint32_t flags, Creature* actor) const
{
	if (const Creature* creature = thing.getCreature()) {
		...
		if (const Monster* monster = creature->getMonster()) {
			bool ignoreBlockCreature = monster->isPathBlockingChecking() && !monster->isFleeing() ||
			...
			if (creatures && !creatures->empty()) {
				for (Creature* tileCreature : *creatures) {
					...
					if (!monster->canSeeInvisibility() && tileCreature->isInvisible()) {
						return RETURNVALUE_NOTPOSSIBLE;
					}
					...
					if (tileCreature->getNpc() ||
					    (!ignoreBlockCreature && !monster->isCreatureAvoidable(tileCreature))) {
						return RETURNVALUE_NOTPOSSIBLE;
					}

					const Monster* creatureMonster = tileCreature->getMonster();
					if (!ignoreBlockCreature && !creatureMonster || monster->getParent() == nullptr ||
					    !tileCreature->isPushable()) { // NPC or spawning summon or non-pushable creature
						return RETURNVALUE_NOTPOSSIBLE;
					}
			...
			if (const TileItemVector* items = getItemList()) {
				for (auto it = items->rbegin(), end = items->rend(); it != end; ++it) {
					if (MagicField* field = (*it)->getMagicField()) {
						...
						if (monster->isFleeing() || monster->State != STATE::PANIC) {
		...
		if (itemIsHangable && hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
			...
			if (items) {
				...
				for (const Item* tileItem : *items) {
					...
					if (itemType.isSplash() && iiType.alwaysOnTopOrder == itemType.alwaysOnTopOrder &&
					    !iiType.isSplash()) {
						return RETURNVALUE_NOTENOUGHROOM;
					}
...
ReturnValue Tile::queryRemove(const Thing& thing, uint32_t count, uint32_t flags, Creature* actor) const
{
...
Tile* Tile::queryDestination(int32_t& index, const Thing& thing, Item** destItem, uint32_t& flags)
{
	if (house) {
		if (const Creature* creature = thing.getCreature()) {
			if (const Player* player = creature->getPlayer()) {
				if (!house->isInvited(player)) {
					...
					if (!destTile) {
						std::cout << "Error: [Tile::queryDestination] House entry not correct"
		...
		if (itemType.isGroundTile()) {
			...
			if (itemType.isSplash() && items) {
				...
				for (ItemVector::const_iterator it = items->getBeginTopItem(), end = items->getEndTopItem(); it != end;
				     ++it) {
					Item* nextItem = *it;
					if (Item::items[nextItem->getID()].alwaysOnTopOrder ==
					        Item::items[item->getID()].alwaysOnTopOrder &&
					    Item::items[item->getID()].isSplash()) {
			...
			if (itemType.isMagicField() || itemType.replaceMagicFields) {
				...
				if (items) {
					for (ItemVector::const_iterator it = items->getBeginDownItem(), end = items->getEndDownItem();
						...
						if (oldField) {
							if (!oldField->canDecay() && !oldField->getDamage() ||
							    item->specialCorpseDrop && !oldField->canDecay()) {
                               ...
                               cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	if (link == LINK_OWNER) {
		...
		} else if (hasFlag(TILESTATE_TELEPORT)) {
...
void Tile::updateHouse(Item* item)
{
	if (item->getParent() != this || !house) {
		return;
	}
...
void Tile::makeRefreshItemList()
{
	...
	if (hasFlag(TILESTATE_REFRESH)) {
		...
		if (refreshableItems) {
			// clear in case of map patch occuring
			refreshableItems->clear();
...
void Tile::refresh()
{
	...
	if (!creatures.empty() && hasFlag(TILESTATE_BLOCKSOLID)) {
		CreatureVector creatureCopy = creatures;
		for (Creature* c : creatureCopy) {
			if (c->getSpeed() != 0) {
				Position pos = c->getPosition();
				if (g_game.searchFreeField(c, pos.x, pos.y, pos.z, 1, false, false)) {
					g_game.internalTeleport(c, pos, false);
				} else {
					if (Monster* monster = c->getMonster()) {
						g_game.internalTeleport(monster, monster->getMasterPos(), false);
					} else if (Npc* npc = c->getNpc()) {
						g_game.internalTeleport(npc, npc->getMasterPos(), false);
					}
				}
			}
		}
