...
void ServiceManager::stop()
{
	...
	for (auto& servicePortIt : acceptors) {
		try {
			boost::asio::post(std::bind(&ServicePort::onStopServer, servicePortIt.second));
	...
	try {
		if (g_config.getBoolean(ConfigManager::BIND_ONLY_GLOBAL_ADDRESS)) {
			acceptor.reset(new boost::asio::ip::tcp::acceptor(
			    io_context, boost::asio::ip::tcp::endpoint(boost::asio::ip::address(boost::asio::ip::make_address(
		...
		pendingStart = true;
		g_scheduler.addEvent(createSchedulerTask(
		    15000, std::bind(&ServicePort::openAcceptor, std::weak_ptr<ServicePort>(shared_from_this()), port)));
	}
}
