...
void Container::addItemFront(Item* item)
{
...
void Container::onRemoveContainerItem(uint32_t index, Item* item)
{
	...
	for (Creature* spectator : spectators) {
		if (size() >= capacity()) {
			...
			spectator->getPlayer()->sendContainer(player->getContainerID(this), this, hasParent());
		} else {
                                ...
                                Creature* actor /* = nullptr*/) const
{
	...
	if (actor && g_config.getBoolean(ConfigManager::ONLY_INVITED_CAN_MOVE_HOUSE_ITEMS)) {
		if (const House* house = topParent->getTile()->getHouse()) {
	...
	if (const DepotLocker* depotLocker = getHoldingDepot()) {
		bool addCheck = true;
		if (item->isStackable()) {
		...
		if (item->getHoldingDepot() != depotLocker) {
			if (depotLocker->getItemHoldingCount() > depotLocker->getMaxDepotItems()) {
				return RETURNVALUE_DEPOTISFULL;
			}
		}

		if (addCheck && (item->getHoldingDepot() != depotLocker || item->getItemCount() != count)) {
			return depotLocker->queryAdd(INDEX_WHEREEVER, *item, count, flags | FLAG_CHILDISOWNER, actor);
		}
                                     ...
                                     uint32_t flags) const
{
	...
	const DepotLocker* itemDepot = item->getHoldingDepot();
	const DepotLocker* thisDepot = getHoldingDepot();
	if (!thisDepot) {
		thisDepot = getDepotLocker();
	}

	if (thisDepot && !itemDepot) {
		if (thisDepot->getItemHoldingCount() + 1 > thisDepot->getMaxDepotItems() ||
		    thisDepot->getItemHoldingCount() >= thisDepot->getMaxDepotItems()) {
			freeSlots = 0;
		}
	}

	if (thisDepot && itemDepot) {
		if (thisDepot->getItemHoldingCount() >= thisDepot->getMaxDepotItems()) {
			const Item* destItem = getItemByIndex(index);
			if (destItem && destItem->equals(item) && destItem->getItemCount() < 100 &&
			    destItem->getItemCount() + count > 100 && item->getItemCount() != count) {
				freeSlots = 0;
			}
		}
	}
