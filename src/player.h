...
static constexpr int32_t PLAYER_FIGHT_DURATION = 60 * 1000;
...
public:
	...
	{
		if (!group) { // player was preloaded internally, not actually logged in
			return false;
		}
	...
	void addStringStorageValue(const std::string& key, const std::string& value);
	bool getStorageValue(const uint32_t key, int32_t& value) const;
	bool getStringStorageValue(const std::string& key, std::string& value) const;
	...
	DepotLocker* getDepotLocker(uint32_t depotId, bool force);
	void onReceiveMail() const;
	bool isNearDepotBox(int32_t depotId = -1) const;

	/// <summary>
	/// Load the contents of the given depot ID and their content begins decaying.
	/// This function should be called before opening any depot lockers in game.
	/// </summary>
	/// <param name="depotId"></param>
	void loadDepotLocker(uint32_t depotId);

	/// <summary>
	/// Unloads the contents of the given depot ID and their content stops decaying.
	/// This function should be called when stepping out of any depot lockers in game.
	/// </summary>
	/// <param name="depotId"></param>
	void unloadDepotLocker(uint32_t depotId);
	...
	void onAttackedCreatureDisappear(bool) override;
	...
	void setLastAttackBlockType(BlockType_t blocktype) { lastAttackBlockType = blocktype; }
	...
	int32_t getBloodHitCount() const { return bloodHitCount; }
	...
	uint32_t getMaxDepotItems() const;
	...
	void sendCreatureSay(uint32_t statementId, const Creature* creature, SpeakClasses type, const std::string& text,
	                     const Position* pos = nullptr)
	{
		if (client) {
			client->sendCreatureSay(statementId, creature, type, text, pos);
		}
	}
	...
	void sendToChannel(uint32_t statementId, const Creature* creature, SpeakClasses type, const std::string& text,
	                   uint16_t channelId) const
	{
		if (client) {
			client->sendToChannel(statementId, creature, type, text, channelId);
		}
	}
...
private:
	...
	std::unordered_map<std::string, std::string> stringStorageMap;
	...
	DepotLocker* currentDepotItem = nullptr;
	...
	uint32_t level = 1;
	uint32_t magLevel = 0;
	int64_t MessageBufferTicks = 0;
	uint32_t MutingEndRound = 0;
	uint32_t lastIP = 0;
	...
	int32_t getStepSpeed() const override { return getSpeed(); }
	void updateBaseSpeed()
	{
		if (!hasFlag(PlayerFlag_SetMaxSpeed) && vocation) {
